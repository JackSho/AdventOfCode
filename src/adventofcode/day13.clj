...
(defn transform
  [[len pos pos-fn :as item]]
  (if (> len 1)
    (let [next-pos    (pos-fn pos)
          next-pos-fn (cond
                        (zero? next-pos) inc
                        (= (dec len) next-pos) dec
                        :default pos-fn)]
      [len next-pos next-pos-fn])
    item))

(defn move
  [data-map]
  (reduce (fn [m [no item]]
            (assoc m no (transform item)))
          {} data-map))
...
(defn fewest-picoseconds
  ...
  (let [input-map   (str->map input-str)
    ...
    (loop [coll  ite-coll
           delay 0]
      (if (not-caught (take sub-seq-len coll))
        delay
        (recur (rest coll) (inc delay))))))
